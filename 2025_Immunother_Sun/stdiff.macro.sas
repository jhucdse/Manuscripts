...
* Calculate standardized differences (d) of means and/or proportions between two groups; 
* Required entry fields:
* Dataset = dataset containing variables; 
* Group = the group variable (i.e., treatment or control) which 
...
(variables must be entered in a list separated by spaces like this: mean=var1 var2 var3 var4); 
* Proportion = list of variables for calculating differences of proportions 
(variables must be entered in a list separated by spaces like this: proportion=var1 var2 var3 var4) 
Variables must be coded as 0/1; 
* Optional fields: 
* W = weight variable (i.e., to weight the sample); 
/*****************************************************************************************************************/
%macro stdiff(dataset=, group=, mean=, proportion=, w=); 
...
%if %length(&mean.) NE 0 %then %do; 
    ...
    data _null_; 
      set _mean_&v_m.; 
      if &group.=0 and _STAT_='MEAN' then call 
      symput("_mean0", &v_m.);  
      else if &group.=0 and _STAT_='STD' then call 
      symput("_sd0", &v_m.); 
      else if &group.=1 and _STAT_='MEAN' then call 
      symput("_mean1", &v_m.); 
      else if &group.=1 and _STAT_='STD' then call 
      symput("_sd1", &v_m.); 
    run;
...
%if %length(&proportion.) NE 0 %then %do; 
    ...
    %do var_prop=1 %to &prop_cnt.; 
      ...
      data _null_; 
      set _prop_&v_p.; 
      if &group.=0 and &v_p.=1 then call 
      symput("_prop0", pct_col);
      else if &group.=1 and &v_p.=1 then call 
      symput("_prop1", pct_col); 
      run; 
