...
#### Version 1: Brown V et al. case definition: 
      ...
      - V1 Criteria 2. There is Rx for new corticosteroid or other irAE tx within +/- 40 days of Dx date 
...
#### Algorithms (all except 3): Merge list of "IRAE" dx with list of ICI patients (with irAE dx after ICI index date).
...
### Algorithms (all except 3) Mark first "IRAE" dx and create running count of encounters
...
#### Algorithms (all except 3): Flag specific AE toxicities for sub-analysis (pneumonitis, colitis, hepatitis)
...
ICI_IRAE_pneumcolhep_t <- bind_rows(ICI_IRAE_pneumcolhep[[1]], 
                         ...
                         ICI_IRAE_pneumcolhep[[5]], ICI_IRAE_pneumcolhep[[5]], 
...
dim(ICI_IRAE_pneumcolhep_t) ## 4357 records
...
### Algorithms (all except 2): irAE treatment Rx (Corticosteroids, other tx that are guideline recommended)
** Algorithm 1, 5: within +/- 40 days of "IRAE" Dx date
** Algorithm 3: any Rx (no dx requirement)
** Algorithm 4: Anytime before "irAE" Dx date or up to 10 days after
** Algorithm 6: TBD
...
irae_tx <- data.frame(code = c("PREDNISONE", "METHYLPREDNISOLONE", 
                               "INFLIXIMAB", 
                               "TRIAMCINOLONE", "BETAMETHASONE", "CLOBETASOL", "DIFLORASONE",  "FLUOCINONIDE", "FLURANDRENOLIDE", "HALOBETASOL", "AMCINONIDE", "DESOXIMETASONE", "DIFLORASONE", "HALCINONIDE", "FLUTICASONE", "FLUOCINOLONE", "MOMETASONE", "ALCLOMETASONE", "DESONIDE", 
                               "LEVOTHYROXINE"))
dim(irae_tx) # 20
...
# cortico_search_a <- function(cortico){
#   cortico_match <- admin_medsdf[grep(pattern=cortico, x=admin_medsdf$medication_name), ]
...
# cortico_rx_a <- apply(cortico, 1, cortico_search_a, simplify=TRUE)
# cortico_rx_at <- bind_rows(cortico_rx_a[[1]], cortico_rx_a[[2]], cortico_rx_a[[3]])
...
irae_tx_at <- bind_rows(irae_tx_a[[1]], irae_tx_a[[2]], irae_tx_a[[3]],
                        ...
                        irae_tx_a[[10]], irae_tx_a[[11]], irae_tx_a[[12]],
                        irae_tx_a[[13]], irae_tx_a[[14]], irae_tx_a[[15]],
                        irae_tx_a[[16]], irae_tx_a[[17]], irae_tx_a[[18]],
                        irae_tx_a[[19]], irae_tx_a[[20]])
...
irae_tx_at <- irae_tx_at %>%
  mutate(nontopical = ifelse(
    (medication_name %in% irae_tx_at[grep("TRIAMCINOLONE", irae_tx_at$medication_name), ]$medication_name & route !="Topical" |
     medication_name %in% irae_tx_at[grep("BETAMETHASONE", irae_tx_at$medication_name), ]$medication_name & route !="Topical" |
     medication_name %in% irae_tx_at[grep("CLOBETASOL", irae_tx_at$medication_name), ]$medication_name & route !="Topical" |
     medication_name %in% irae_tx_at[grep("DIFLORASONE", irae_tx_at$medication_name), ]$medication_name & route !="Topical" |
     medication_name %in% irae_tx_at[grep("FLUOCINONIDE", irae_tx_at$medication_name), ]$medication_name & route !="Topical" |
     medication_name %in% irae_tx_at[grep("FLURANDRENOLIDE", irae_tx_at$medication_name), ]$medication_name & route !="Topical" |
     medication_name %in% irae_tx_at[grep("HALOBETASOL", irae_tx_at$medication_name), ]$medication_name & route !="Topical" |
     medication_name %in% irae_tx_at[grep("AMCINONIDE", irae_tx_at$medication_name), ]$medication_name & route !="Topical" |
     medication_name %in% irae_tx_at[grep("DESOXIMETASONE", irae_tx_at$medication_name), ]$medication_name & route !="Topical" |
     medication_name %in% irae_tx_at[grep("DIFLORASONE", irae_tx_at$medication_name), ]$medication_name & route !="Topical" |
     medication_name %in% irae_tx_at[grep("HALCINONIDE", irae_tx_at$medication_name), ]$medication_name & route !="Topical" |
     medication_name %in% irae_tx_at[grep("FLUTICASONE", irae_tx_at$medication_name), ]$medication_name & route !="Topical" |
     medication_name %in% irae_tx_at[grep("FLUOCINOLONE", irae_tx_at$medication_name), ]$medication_name & route !="Topical" |
     medication_name %in% irae_tx_at[grep("MOMETASONE", irae_tx_at$medication_name), ]$medication_name & route !="Topical" |
     medication_name %in% irae_tx_at[grep("ALCLOMETASONE", irae_tx_at$medication_name), ]$medication_name & route !="Topical" |
     medication_name %in% irae_tx_at[grep("DESONIDE", irae_tx_at$medication_name), ]$medication_name & route !="Topical"), 1, 0))

table(irae_tx_at$nontopical)
...
table(irae_tx_at$nontopical) ## no more nontopical use of topical corticosteroid

### Indicator variable for dose 50mg for prednisone or 40mg for methylprednisolone or 250mg for infliximab (these are threshold doses for irAE treatment per Brahmer guidance) (no threshold for triamcinolone/etc, levothyroxine)

  ## List of all irAE tx
irae_tx_at <- irae_tx_at %>%
  mutate(sig_num = round(as.numeric(sig)),0) %>%
  mutate(pred_50_equiv = ifelse(
    (medication_name %in% irae_tx_at[grep("PREDNISONE", irae_tx_at$medication_name), ]$medication_name & sig_num >=50) 
                                | 
      (medication_name %in% irae_tx_at[grep("METHYLPREDNISOLONE", irae_tx_at$medication_name), ]$medication_name & sig_num >=40) 
                                | 
      (medication_name %in% irae_tx_at[grep("INFLIXIMAB", irae_tx_at$medication_name), ]$medication_name & sig_num >=250) 
                                   | 
      (medication_name %in% irae_tx_at[grep("TRIAMCINOLONE", irae_tx_at$medication_name), ]$medication_name) 
                                   | 
      (medication_name %in% irae_tx_at[grep("LEVOTHYROXINE", irae_tx_at$medication_name), ]$medication_name), 
                                   1, 0))
...
irae_tx_at_50 <- filter(irae_tx_at, pred_50_equiv==1)


dim(irae_tx_at) ## 75261 records for corticosteroid of interest in medication_name field
length(unique(irae_tx_at$cohort_id)) ## 5145 unique patient_id with corticosteroid rx


dim(irae_tx_at_50) ## 50594 records for corticosteroid of interest in medication_name field >50mg
length(unique(irae_tx_at_50$cohort_id)) ## 4362 unique patient_id with corticosteroid rx >50mg
...
# Assess the result
  ...
  View(irae_tx_at)
  dim(irae_tx_at)
  length(unique(irae_tx_at$cohort_id)) # 3932
  
  ## Dose >50mg
  View(irae_tx_at_50)
  dim(irae_tx_at_50)
  length(unique(irae_tx_at_50$cohort_id)) # 3336
...
length(unique(prevalent_irae_tx$cohort_id)) ## 139 prevalent users (any cortico dose)
...
length(unique(prevalent_irae_tx_50$cohort_id)) ## 127 prevalent users (cortico dose >50)
...
# All doses
irae_tx_at_incident <- subset(irae_tx_at, !(cohort_id %in% prevalent_irae_tx$cohort_id))
length(unique(irae_tx_at_incident$cohort_id)) # 3843


# Dose >50mg
irae_tx_at_50_incident <- subset(irae_tx_at_50, !(cohort_id %in% prevalent_irae_tx_50$cohort_id))
length(unique(irae_tx_at_50_incident$cohort_id)) # 3254
...
```{r irae tx admin dedupe}
## Deduplicate list of cortico Rx (where there is multiple records from same date/time/dose)

# All doses
irae_tx_at_dedupe <- irae_tx_at[!duplicated(irae_tx_at[c(1,6,10)]),] 

dim(irae_tx_at_dedupe)
length(unique(irae_tx_at_dedupe$cohort_id)) # 3843

save(irae_tx_at_dedupe, file = "irae_tx_at_dedupe.Rdata")


# Dose >50mg
irae_tx_at_50_dedupe <- irae_tx_at_50[!duplicated(irae_tx_at_50[c(1,6,10)]),] 

dim(irae_tx_at_50_dedupe)
length(unique(irae_tx_at_50_dedupe$cohort_id)) # 3336

save(irae_tx_at_50_dedupe, file = "irae_tx_at_50_dedupe.Rdata")

## Create another table that just counts appearances per person

# All doses
irae_tx_at_dedupe.table <- count(irae_tx_at_dedupe, cohort_id)

# Dose >50mg
irae_tx_at_50_dedupe.table <- count(irae_tx_at_50_dedupe, cohort_id)
...
# All doses
irae_tx_at_dedupe.index <- subset(irae_tx_at_dedupe,first_appearance==1 & RunningCount==1, select =c("cohort_id","medication_name","ordering_date"))

View(irae_tx_at_dedupe.index)

length(unique(irae_tx_at_dedupe.index$cohort_id)) ## 3932 unique cohort_id

## Dose >50mg
irae_tx_at_50_dedupe.index <- subset(irae_tx_at_50_dedupe,first_appearance==1 & RunningCount==1, select =c("cohort_id","medication_name","ordering_date"))

length(unique(irae_tx_at_50_dedupe.index$cohort_id)) ## 3336 unique cohort_id
```
...
load(file = "S:/JamieHeyward/Dissertation Aim 1 PMAP Analysis/R Scripts/For Manuscript/irae_tx_at_dedupe.Rdata")
...
## Create dataframe with just cohort_id and each date of corticosteroid rx
    ...
    # All doses
  irae_tx_at_dedupe.allrx <- subset(irae_tx_at_dedupe, select =c("cohort_id","medication_name","sig_num","dose_unit","ordering_date","RunningCount"))
  
  # Dose >50mg
  irae_tx_at_50_dedupe.allrx <- subset(irae_tx_at_50_dedupe, select =c("cohort_id","medication_name","sig_num","dose_unit","ordering_date","RunningCount"))

  
## Filter corticosteroid list on membership in dx / ICI list
  
    # All doses
  irae_tx_all_IRAEpat <- irae_tx_at_dedupe.allrx[irae_tx_at_dedupe.allrx$cohort_id %in% ICI_IRAE_1_dedupe$cohort_id, ] 
  
  length(unique(irae_tx_all_IRAEpat$cohort_id)) ## 399 unique patient ID with corticosteroid on ICI/"IRAE" list, this is the list of unique patients and their corticosteroid index date who also were on ICI and have a emr encounter ICD code for dx that might be IRAE
  
  # Dose >50mg
  irae_tx_all_IRAEpat_50 <- irae_tx_at_50_dedupe.allrx[irae_tx_at_50_dedupe.allrx$cohort_id %in% ICI_IRAE_1_dedupe$cohort_id, ] 
  
  length(unique(irae_tx_all_IRAEpat_50$cohort_id)) ## 356 unique patient ID with corticosteroid on ICI/"IRAE" list, this is the list of unique patients and their corticosteroid index date who also were on ICI and have a emr encounter ICD code for dx that might be IRAE
```
...
## Filter on Rx DATE less than 10-day buffer period before or after dx date
  ...
  dim(irae_tx_all_IRAE_1) # 999 individual records, meaning possibly multi-system IRAE or multiple successive IRAE in the same patient
    
  length(unique(irae_tx_all_IRAE_1$cohort_id)) ## 315 unique patient ID
  ...
  dim(irae_tx_all_IRAE_1_50) # 773 individual records, meaning possibly multi-system IRAE or multiple successive IRAE in the same patient
    
  length(unique(irae_tx_all_IRAE_1_50$cohort_id)) ## 283 unique patient ID
...
## Filter on corticosteroid INDEX date anytime before "IRAE" dx
  ...
  length(unique(irae_tx_IRAE4$cohort_id)) ## 326
  ...
  length(unique(irae_tx_IRAE4_50$cohort_id)) ## 289
...
dim(irae_tx_all_IRAE_1) ## 5716     
dim(irae_tx_all_IRAE_1_rechalleng_excl) ## 5511  

length(unique(irae_tx_all_IRAE_1$cohort_id)) ## 288 unique patients
length(unique(irae_tx_all_IRAE_1_rechalleng_excl$cohort_id)) ## 275 unique patients - wow

dim(irae_tx_all_IRAE_1_50) ## 4396      
dim(irae_tx_all_IRAE_1_50_rechalleng_excl) ## 4226    

length(unique(irae_tx_all_IRAE_1_50$cohort_id)) ## 258 unique patients
length(unique(irae_tx_all_IRAE_1_50_rechalleng_excl$cohort_id)) ## 248 unique patients - wow
...
length(unique(irae_tx_all_IRAE_1_rechalleng_excl_pc$cohort_id)) ## 155 unique patients
...
length(unique(irae_tx_all_IRAE_1_50_rechalleng_excl_pc$cohort_id)) ## 140 unique patients
